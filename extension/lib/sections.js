...
export const SECTION_DEFAULTS = {
  ...
  // Audit 3 (melodii reale): fără astea, Let It Be ieșea „Partea 1…7” (9 secțiuni), iar
  // Wonderwall 16 secțiuni cu multe „Trecere”. Vezi consolidateSections.
  maxGroups: 4,          // câte tipare distincte arătăm cel mult (strofă, refren, punte, +1)
  // Similaritate PONDERATĂ pe timp (Σmin/Σmax al fracțiunilor de timp per acord) peste care
  // două bucăți sunt „același material”. Strofa și refrenul clasic (G D Am C față de Em C G D)
  // dau 0,60 — deci rămân separate; aceeași buclă cu doi intruși dă peste 0,8.
  sameMaterial: 0.66,
  meltTransition: 12,    // o zonă liberă mai scurtă de atât, între două secțiuni ale aceluiași grup, se topește
};

/**
 * Curăță intrușii ÎNAINTE de detecția structurii (și pentru tot ce se afișează în Pasul 2).
 * Un acord care ține sub `shortFactor` × mediana duratelor ȘI a cărui etichetă acoperă sub
 * `rareShare` din melodie e o detecție de o clipă, nu un acord al melodiei: se scoate, iar
 * timpul lui trece la acordul dinainte. Pe melodii reale (audit 3) intrușii sunt scurți
 * (0,8–1 s la o mediană de 1,5–2 s) și rari; acordurile melodiei sunt frecvente. Un acord de
 * trecere REAL, scurt și rar, cade și el — preț mic față de „Partea 1…7” și foaia cu 174 de
 * chip-uri. Memoria salvată rămâne brută; curățarea e doar pentru afișare.
 * @param {{t:number,label:string}[]} chords
 * @param {number} duration
 */
export function cleanTimeline(chords, duration, opts = {}) {
  const o = { shortFactor: 0.6, rareShare: 0.03, ...opts };
  const events = (chords || [])
    .filter((c) => c && Number.isFinite(c.t) && typeof c.label === 'string')
    .sort((a, b) => a.t - b.t);
  const n = events.length;
  if (n < 4) return events.slice();
  const gaps = [];
  for (let i = 1; i < n; i++) gaps.push(events[i].t - events[i - 1].t);
  gaps.sort((a, b) => a - b);
  const median = gaps[Math.floor(gaps.length / 2)] || 1;
  const end = Number.isFinite(duration) && duration > events[n - 1].t ? duration : events[n - 1].t + median;
  const dur = (i) => (i + 1 < n ? events[i + 1].t : end) - events[i].t;
  const timePer = new Map();
  for (let i = 0; i < n; i++) timePer.set(events[i].label, (timePer.get(events[i].label) || 0) + dur(i));
  const keep = [];
  for (let i = 0; i < n; i++) {
    const e = events[i];
    const intruder = dur(i) < o.shortFactor * median && (timePer.get(e.label) || 0) < o.rareShare * end;
    if (!intruder) keep.push(e);
  }
  if (!keep.length) return events.slice();
  // Primul acord scos: următorul păstrat îi moștenește momentul — nu lăsăm gaură la început.
  if (keep[0] !== events[0]) keep[0] = { ...keep[0], t: events[0].t };
  // Vecinii cu aceeași etichetă se contopesc (după scoaterea unui intrus dintre două A-uri).
  const out = [];
  for (const e of keep) {
    const last = out[out.length - 1];
    if (last && last.label === e.label) continue;
    out.push(e);
  }
  return out;
}
...
export function detectSections(chords, duration, opts = {}) {
  ...
  sections = mergeAdjacentSameLetter(sections);
  sections = consolidateSections(sections, clusters, seq, dt, o);
  o._debug?.('sectiuni-consolidate', sections.map((s) => ({ ...s })));

  nameSections(sections, (a, b) => sameMaterial(
    labelShare(seq, Math.round(a.start / dt), Math.round(a.end / dt)),
    labelShare(seq, Math.round(b.start / dt), Math.round(b.end / dt)),
  ) >= o.sameMaterial);

  const patterns = {};
  for (const c of clusters) {
    if (c.letter === null || c.merged) continue;
    const { loop, lead } = compressLoop(c.loopSeq, dt);
    patterns[c.letter] = { loop, period: c.period * dt, lead };
  }

  // Acoperirea numără și strofele „libere” numite mai sus: sunt structură, nu zgomot.
  let covered = 0;
  for (const s of sections) if (s.cluster || s.name === 'verse') covered += s.end - s.start;
...
function mergeAdjacentSameLetter(sections) {
...
// Cât din timpul unei felii sună fiecare acord (fracțiuni care dau 1 împreună).
function labelShare(seq, s, e) {
  const share = new Map();
  let total = 0;
  for (let j = Math.max(0, s); j < Math.min(seq.length, e); j++) {
    if (seq[j] === null) continue;
    share.set(seq[j], (share.get(seq[j]) || 0) + 1);
    total++;
  }
  for (const [k, v] of share) share.set(k, v / Math.max(1, total));
  return share;
}

// „Același material?” — Jaccard PONDERAT pe timp: Σ min / Σ max al fracțiunilor de timp.
// Pe seturi simple, un intrus de 0,8 s conta cât un acord de 10 s și două strofe identice
// ieșeau „diferite” din cauza a doi intruși; ponderat, intrușii cântăresc cât durează.
...
// Ponderile unei bucle (tiparul-consens al unui grup), în aceleași unități.
function loopShare(loopSeq) {
  return labelShare(loopSeq, 0, loopSeq.length);
}

// CONSOLIDAREA (audit 3, melodii reale). Detecția de perioade e corectă local, dar pe o
// melodie reală cu 10–15 % intruși fărâmițează: aceeași strofă apare ca trei grupuri cu
// litere diferite (bucla e „un pic altfel” la fiecare trecere), iar între ele răsar zone
// libere de câteva secunde. Trei reguli, în ordinea asta:
//   1. plafon de grupuri: peste `maxGroups`, perechile de grupuri cu cel mai asemănător SET de
//      acorduri (Jaccard) se unesc, până rămân `maxGroups` — litera mai veche câștigă;
//   2. vecine cu același material (Jaccard ≥ `sameMaterial`) se unesc — inclusiv o zonă liberă
//      lipită de o secțiune cu buclă, când acordurile ei sunt tot alea;
//   3. o zonă liberă mai scurtă de `meltTransition` s între două secțiuni ale ACELUIAȘI grup,
//      făcută din acordurile grupului, e o buclă stricată de zgomot, nu o punte: se topește
//      în ele. O punte adevărată aduce acorduri NOI (F, F#m între două refrenuri) și rămâne.
// NU atinge detecția de perioade; lucrează doar pe secțiunile deja găsite.
function consolidateSections(sections, clusters, seq, dt, o) {
  const byLetter = new Map(clusters.filter((c) => c.letter !== null).map((c) => [c.letter, c]));
  const relabel = (from, to) => {
    for (const s of sections) if (s.cluster === from) s.cluster = to;
    const c = byLetter.get(from);
    if (c) { c.merged = true; byLetter.delete(from); }
  };
  const repsOf = (start, end, letter, fallback) => {
    const period = byLetter.get(letter)?.period;
    return period ? Math.max(1, Math.round((end - start) / (period * dt))) : fallback;
  };

  // 0. un „tipar” cu un singur acord e o dronă, nu o buclă: secțiunile lui devin zone libere
  for (const [letter, c] of [...byLetter]) {
    if (new Set(c.loopSeq.filter((x) => x !== null)).size >= 2) continue;
    for (const s of sections) if (s.cluster === letter) { s.cluster = null; s.reps = 1; }
    c.merged = true;
    byLetter.delete(letter);
  }

  // 1. grupuri cu aceeași buclă (rotită sau cu un intrus) = același grup; apoi plafonul:
  //    peste `maxGroups`, cele mai asemănătoare două se unesc, până rămân `maxGroups`.
  for (;;) {
    ...
    for (let i = 0; i < letters.length; i++) {
      for (let j = i + 1; j < letters.length; j++) {
        const a = byLetter.get(letters[i]), b = byLetter.get(letters[j]);
        const sim = sameMaterial(loopShare(a.loopSeq), loopShare(b.loopSeq));
        if (!best || sim > best.sim) best = { sim, from: letters[j], to: letters[i] };
      }
    }
    if (best.sim < o.sameMaterial && letters.length <= o.maxGroups) break;
    relabel(best.from, best.to);
  }
  sections = mergeAdjacentSameLetter(sections);

  // 2. vecine cu același material
  let changed = true;
  while (changed) {
    changed = false;
    for (let i = 0; i + 1 < sections.length; i++) {
      const a = sections[i], b = sections[i + 1];
      if (a.cluster === null && b.cluster === null) continue; // două zone libere rămân libere
      const sa = labelShare(seq, Math.round(a.start / dt), Math.round(a.end / dt));
      const sb = labelShare(seq, Math.round(b.start / dt), Math.round(b.end / dt));
      if (sameMaterial(sa, sb) < o.sameMaterial) continue;
      const host = a.cluster === null ? b : b.cluster === null ? a : (a.end - a.start >= b.end - b.start ? a : b);
      if (a.cluster !== null && b.cluster !== null && a.cluster !== b.cluster) {
        relabel(host === a ? b.cluster : a.cluster, host.cluster);
      }
      sections.splice(i, 2, { start: a.start, end: b.end, cluster: host.cluster, name: null,
        reps: repsOf(a.start, b.end, host.cluster, a.reps + b.reps) });
      changed = true;
      break;
    }
  }
  ...
  // 3. trecerile scurte dintre două secțiuni ale aceluiași grup
  changed = true;
  while (changed) {
    changed = false;
    for (let i = 1; i + 1 < sections.length; i++) {
      const s = sections[i];
      if (s.cluster !== null || s.end - s.start >= o.meltTransition) continue;
      const a = sections[i - 1], b = sections[i + 1];
      if (a.cluster === null || a.cluster !== b.cluster) continue;
      const own = labelShare(seq, Math.round(s.start / dt), Math.round(s.end / dt));
      if (sameMaterial(own, loopShare(byLetter.get(a.cluster)?.loopSeq || [])) < 0.5) continue; // material nou = punte, nu zgomot
      sections.splice(i - 1, 3, { start: a.start, end: b.end, cluster: a.cluster, name: null,
        reps: repsOf(a.start, b.end, a.cluster, a.reps + b.reps) });
      changed = true;
      break;
    }
  }
...
// `similar(a, b)` spune dacă două secțiuni au același material (vine din detectSections).
function nameSections(sections, similar = () => false) {
  ...
  const names = new Map();
  // Melodii reale (audit 3): strofa e deseori prea „liberă” ca să iasă buclă (arpegii,
  // acorduri ținute), dar refrenul iese. Dacă grupul cel mai repetat revine de cel puțin
  // două ori și cel puțin două zone libere lungi (nu ultima — aia e final) repetă ÎNTRE ELE
  // același material, avem strofă (liberă) + refren (buclă).
  let mostRepeated = order[0];
  for (const c of order) if (counts.get(c) > counts.get(mostRepeated)) mostRepeated = c;
  if (counts.get(mostRepeated) >= 2) {
    const free = sections.filter((s, i) => !s.cluster && i < sections.length - 1 && s.end - s.start >= 8);
    const twins = free.filter((a) => free.some((b) => b !== a && similar(a, b)));
    if (twins.length >= 2) {
      names.set(mostRepeated, 'chorus');
      for (const s of twins) s.name = 'verse';
    }
  }
  if (names.size === 0 && order.length >= 2) {
    const first = order[0];
    let top = null;
  ...
  // (O zonă liberă deja numită „strofă” mai sus își păstrează numele.)
  if (!sections[0].cluster && !sections[0].name) sections[0].name = 'intro';
  const last = sections[sections.length - 1];
  if (sections.length > 1 && !last.cluster && !last.name) last.name = 'outro';
  const interior = sections.filter(
    (s, i) => !s.cluster && !s.name && i > 0 && i < sections.length - 1,
  );

...
// Cât trebuie să câștige o poziție de capo față de „fără capo” ca s-o sugerăm: cel puțin
// 15 % din acordurile melodiei trec din barré în forme deschise. Audit 3: pe Knockin' on
// Heaven's Door (G D Am C — toate deschise) se sugera capo 5 pentru un câștig de 1 %; pe Let
// It Be capo 5 rămâne sugerat, fiindcă F-ul cu barré ține 28 % din melodie și devine C.
export const CAPO_MIN_GAIN = 0.15;
...
// din melodie în forme deschise. La egalitate câștigă capo-ul mai mic, iar o poziție > 0
// trebuie să bată „fără capo” cu cel puțin CAPO_MIN_GAIN.
export function bestCapo(labels, maxCapo = 7) {
  ...
  let best = null;
  let none = null; // scorul lui „fără capo”
  for (let capo = 0; capo <= maxCapo; capo++) {
    ...
    const entry = { capo, score, played, total };
    if (capo === 0) none = entry;
    if (!best || score > best.score) best = entry;
  }
  return best.capo > 0 && best.score - none.score < CAPO_MIN_GAIN ? none : best;
}

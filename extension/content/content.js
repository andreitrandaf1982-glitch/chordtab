...
import { tearPage, sheetAsText } from '../lib/tear-page.js';
import {
  sheetRows, chordsBetween as chordsBetweenIn, hasUsefulStructure as structureIsUseful,
  clusterOrdinals, makeSectionLabel,
...
const state = {
  ...
  invoked: null,      // a fost extensia invocată pe tabul ăsta? null = încă nu știm
  contextLost: false, // extensia a fost reîncărcată sub noi (vezi onContextLost)
  opened: 0,          // de câte ori a fost deschisă melodia asta din memorie (pentru Caietul meu)
...
function init() {
  ...
  // loadCache() și askCanStart() sunt chemate de syncPanelPresence() → buildPanel(); nu și
  // aici, altfel memoria se citea de două ori la pornire (și numărul de deschideri din
  // Caietul meu creștea cu 2).
}
...
// Când extensia e reîncărcată sau actualizată din chrome://extensions, content scriptul din
// taburile deja deschise rămâne orfan: `chrome.runtime.id` devine undefined și orice apel
// chrome.* aruncă „Extension context invalidated” — eroarea pe care a văzut-o Andrei în
// pagina Errors a extensiei (21.09.2026), din saveCache, după un reload. Nu e o defecțiune a
// analizei; e Chrome care ne spune că pagina asta a rămas cu versiunea veche. Verificăm
// ÎNAINTE de fiecare apel și, la prima pierdere, oprim tot și spunem omului să dea refresh.
...
function onContextLost() {
  ...
  if (ui.status) {
    ui.status.textContent = STR.reloaded;
    ui.status.classList.add('is-warning');
  }
  if (ui.action) ui.action.disabled = true;
  // Fără log.warn: mesajul ar ajunge tot în pagina Errors a extensiei, ca un defect.
}

/** chrome.runtime.sendMessage care nu aruncă niciodată: contextul pierdut oprește panoul. */
function send(msg) {
  if (!contextAlive()) { onContextLost(); return Promise.resolve(undefined); }
  try {
...
function onNavigate() {
  ...
  Object.assign(state, {
    ...
    lastIndex: -1, lastSection: -1, opened: 0,
  });
  syncPanelPresence();
}

// `tabCapture` cere ca extensia să fi fost INVOCATĂ pe tab (click pe iconiță / Alt+Shift+C).
// Butonul din panou nu poate ține loc de gest — verificat pe YouTube real. Întrebăm
// background-ul și, dacă tabul nu e invocat, panoul trimite la iconiță ÎNAINTE de un eșec.
function askCanStart() {
  send({ target: 'background', type: 'CAN_START' }).then((r) => {
    if (r === undefined) return; // fără răspuns: rămânem pe „nu știm”, butonul e cel obișnuit
    const was = state.invoked;
    state.invoked = !!r?.invoked;
    if (was !== state.invoked && ui.panel) render();
  });
}
...
async function loadCache() {
  ...
  if (!contextAlive()) { onContextLost(); return; }
  try {
    ...
    // Caietul meu: numărăm deschiderea și completăm titlul (memoriile vechi n-au titlu).
    state.opened = (stored.opened || 0) + 1;
    chrome.storage.local.set({
      [key]: { ...stored, opened: state.opened, title: stored.title || songTitle() },
    }).catch(() => {});
    if (id !== state.videoId) return; // o navigare între timp: nu pornim redarea pe alt video
    startPlayback();
  } catch (err) {
  ...
  try {
    ...
    await chrome.storage.local.set({
      [cacheKey()]: {
        ...
        chords: state.chords,
        title: songTitle(),      // pentru Caietul meu
        opened: state.opened,    // se păstrează la reanaliză
      },
    });
...
function displayLabel(sounding) {
  // „N.C.” (niciun acord clar) se arată ca „—”: nu e un acord de cântat, deci nici diagramă.
  if (!sounding || sounding === NO_CHORD) return STR.noChordsYet;
...
function buildPanel() {
  ...
  // Filtrul „linie tremurată” al temei caiet: diagrama și bara structurii trec prin el ca
  // liniile drepte să tremure ușor, ca desenate cu mâna. E un <svg> de 0×0, definit aici
  // (nu în CSS) fiindcă un filtru referit cu url(#id) trebuie să existe în DOM-ul paginii.
  panel.innerHTML = `
    <svg class="ct-defs" width="0" height="0" aria-hidden="true"><filter id="ct-rough" x="-2%" y="-2%" width="104%" height="104%"><feTurbulence type="fractalNoise" baseFrequency="0.04" numOctaves="2" seed="7" result="n"/><feDisplacementMap in="SourceGraphic" in2="n" scale="1.8" xChannelSelector="R" yChannelSelector="G"/></filter></svg>
    <div class="ct-head">
      <span class="ct-title">${STR.title}</span>
      <span class="ct-status"></span>
      <button class="ct-notebook" type="button" title="${STR.notebookHelp}" aria-label="${STR.notebook}">
        <svg viewBox="0 0 32 32" width="30" height="30" aria-hidden="true">
          <rect x="7" y="3" width="21" height="26" rx="2.5" fill="#f54f1b"/>
          <rect x="9.5" y="5.5" width="16" height="21" rx="1.5" fill="#f6f1e4"/>
          <path d="M12 11h11M12 15h11M12 19h8" stroke="#1e223d" stroke-width="1.3" stroke-linecap="round" opacity=".7"/>
          <rect x="21" y="3" width="4" height="9" fill="#3058f0"/>
          <g fill="#1e223d"><circle cx="7" cy="8" r="1.8"/><circle cx="7" cy="14" r="1.8"/><circle cx="7" cy="20" r="1.8"/><circle cx="7" cy="26" r="1.8"/></g>
          <g stroke="#f6f1e4" stroke-width="1.2" fill="none"><path d="M5 8h4M5 14h4M5 20h4M5 26h4"/></g>
        </svg>
        <span class="ct-notebook-label">${STR.notebook}</span>
      </button>
    ...
    <div class="ct-sheet-wrap" hidden>
      <div class="ct-sheet-head">
        <span class="ct-group-label">${STR.sheet}</span>
        <button class="ct-btn ct-copy" type="button" title="${STR.copyHelp}">${STR.copySheet}</button>
        <button class="ct-btn ct-tear" type="button" title="${STR.tearHelp}">${STR.tearPage}</button>
      </div>
  ...
  Object.assign(ui, {
    ...
    copy: panel.querySelector('.ct-copy'),
    tear: panel.querySelector('.ct-tear'),
    notebook: panel.querySelector('.ct-notebook'),
  });
  ...
  ui.copy.addEventListener('click', onCopySheet);
  ui.tear.addEventListener('click', onTearPage);
  ui.notebook.addEventListener('click', () => { send({ target: 'background', type: 'OPEN_NOTEBOOK' }); });
  buildCapoButtons();
  buildSpeedButtons();
  buildGuide();
...
function render() {
  ...
  // Tab neinvocat, melodie neanalizată: butonul din panou n-ar putea porni nimic, deci
  // trimite la iconiță. Pe o melodie deja memorată butonul rămâne „Analizează din nou” —
  // n-are ce „porni”; dacă e apăsat pe un tab neinvocat, onActionClick explică gestul.
  const needsIcon = state.invoked === false && mode === 'idle';
  ui.action.textContent =
    mode === 'listening' ? STR.stop :
    needsIcon ? STR.startFromIcon :
    mode === 'playback' ? STR.reanalyze : STR.start;
  ui.action.classList.toggle('is-icon-hint', needsIcon);
  ui.action.disabled = state.contextLost;
...
function toggleGuide(open = ui.guide.hidden, remember = true) {
  ...
  if (remember && !open && contextAlive()) chrome.storage.local.set({ guideSeen: true }).catch(() => {});
}
...
function renderStep() {
  ...
  ui.step.textContent =
    ...
    state.mode === 'playback'
      ...
      : state.invoked === false ? STR.stepIdleFirst : STR.stepIdle;
...
function sectionLabel(s) {
  return makeSectionLabel(STR, state.clusterOrder)(s);
}

function videoDuration() {
  const video = getVideo();
  return video && Number.isFinite(video.duration) && video.duration > 0 ? video.duration : Infinity;
}
...
function hasUsefulStructure() {
  return structureIsUseful(state.structure);
}
...
function chordsBetween(from, to) {
  return chordsBetweenIn(state.chords, from, to);
}
...
/**
 * Modelul foii — UN singur loc din care se construiesc rândurile din panou, textul copiat și
 * pagina ruptă (imaginea). Logica e în lib/sheet-model.js (pură), ca s-o folosească și
 * Caietul meu; aici doar îi dăm cronologia, structura și numele secțiunilor.
 */
function sheetModel() {
  return sheetRows(state.chords, state.structure, {
    sectionLabel, wholeSong: STR.wholeSong, rowAt: STR.sheetRowAt,
  });
}

/** Titlul melodiei, din pagina YouTube (fără coada „ - YouTube”). */
function songTitle() {
  const h = document.querySelector('h1.ytd-watch-metadata, #title h1, h1.title')?.textContent?.trim();
  // document.title poartă și „(3) ” de la notificări și coada „ - YouTube”: le scoatem.
  const t = h || document.title.replace(/^\(\d+\)\s*/, '').replace(/\s*-\s*YouTube\s*$/i, '').trim();
  return t || 'Melodie';
}

/** Ce primesc textul copiat și pagina ruptă: modelul foii + titlu, capo, ton, dată. */
function exportModel() {
  const d = state.analyzedAt ? new Date(state.analyzedAt) : null;
  return {
    title: songTitle(),
    capo: state.capo,
    transpose: state.transpose,
    analyzedAt: d && !Number.isNaN(d.getTime()) ? d.toLocaleDateString('ro-RO') : null,
    rows: sheetModel().map((r) => ({
      label: r.label, group: r.group, reps: r.reps,
      chips: r.chips.map((c) => displayLabel(c.label)),
    })),
  };
}

// Feedback scurt pe buton („Copiat ✓”), apoi textul lui de dinainte.
function flashButton(btn, text) {
  const before = btn.dataset.label || btn.textContent;
  btn.dataset.label = before;
  btn.textContent = text;
  clearTimeout(btn._flash);
  btn._flash = setTimeout(() => { btn.textContent = before; }, 1600);
}

async function onCopySheet() {
  if (state.mode !== 'playback') return;
  const text = sheetAsText(exportModel());
  try {
    await navigator.clipboard.writeText(text);
    flashButton(ui.copy, STR.copied);
    log.info('Foaia copiată ca text.');
  } catch (err) {
    log.warn('Nu am putut copia foaia:', err?.message);
    flashButton(ui.copy, STR.copyFailed);
  }
}

async function onTearPage() {
  if (state.mode !== 'playback') return;
  try {
    const name = await tearPage(exportModel());
    flashButton(ui.tear, STR.torn);
    log.info('Pagina ruptă:', name);
  } catch (err) {
    log.error('Nu am putut rupe pagina:', err?.message || err);
    flashButton(ui.tear, STR.copyFailed);
  }
}
...
function buildSheet() {
  ...
  sheetModel().forEach((row, i) => {
    // Momentele chip-urilor se memorează pentru rândurile FĂRĂ tipar (zone libere, foaia
    // plată): evidențierea le caută aici, nu în DOM, la fiecare cadru.
    const isPattern = st && st.sections[i]?.cluster && st.patterns[st.sections[i].cluster];
    if (!isPattern) ui.sheetTimes.set(i, row.chips.map((c) => c.at));
    ui.sheet.appendChild(sheetRow({
      tag: { text: row.label, start: row.start },
      group: row.group,
      chips: row.chips.map((c) => makeSheetChip(c.label, c.at)),
      reps: row.reps,
      index: i,
      range: row.range,
    }));
...
/** Pe foaia plată: rândul (felia de SHEET_FLAT_ROW acorduri) în care cade momentul t. */
function flatRowAt(t) {
  const times = ui.sheetTimes;
  if (!times) return 0;
  let row = 0;
  for (const [r, list] of times) if (list.length && list[0] <= t && r > row) row = r;
  return row;
}

function sheetChipIndexAt(section, t, rowIdx = 0) {
  const st = state.structure;
  ...
  const times = ui.sheetTimes?.get(section ? sectionIndexOf(section) : rowIdx);
...
function updateSheetHighlight(t) {
  ...
  const st = hasUsefulStructure() ? state.structure : null;
  const rowIdx = st ? sectionIndexAt(t) : flatRowAt(t);
  const section = st ? st.sections[rowIdx] : null;
  const chipIdx = sheetChipIndexAt(section, t, rowIdx);
...
function onActionClick() {
  if (!contextAlive()) { onContextLost(); return; }
  if (state.mode === 'listening') {
    send({ target: 'background', type: 'REQUEST_STOP' });
    return;
  }
  // Tab neinvocat: cererea ar eșua garantat (activeTab). Pe o melodie neanalizată deschidem
  // ghidul și pulsăm linia pasului, care spune exact ce e de făcut; pe una memorată spunem
  // în linia de stare unde e iconița. Nu-i un ocol al regulii Chrome — e adevărul.
  if (state.invoked === false) {
    if (state.mode === 'idle') toggleGuide(true, false);
    else {
      ui.status.textContent = STR.needIconClick;
      ui.status.classList.add('is-warning');
    }
    ui.step.classList.remove('is-pulse');
    void ui.step.offsetWidth; // repornește animația și la al doilea click
    ui.step.classList.add('is-pulse');
    return;
  }
  // Cine apasă „Analizează” a înțeles ce are de făcut — nu-i mai deschidem ghidul.
  chrome.storage.local.set({ guideSeen: true }).catch(() => {});
  send({ target: 'background', type: 'REQUEST_START' });
}
...
function onVideoEnded() {
  ...
  send({ target: 'background', type: 'REQUEST_STOP' });
  const video = state.clockVideo; // stopClock() îl uită
...
// Autoplay-ul YouTube fura Pasul 2: la `ended` pornea numărătoarea „Urmează în 8” și după
// 8 s naviga la altă melodie, iar panoul abia trecut în Pasul 2 se reseta (senzația lui
// Andrei: „trebuie să dau back”). Verificat pe YouTube real (21.09.2026): o derulare la 0
// anulează numărătoarea, dar playerul RELUĂ redarea după derulare, deci o pauză dată imediat
// e călcată — pauza se dă la primul `playing`. Cu Loop nativ nu există numărătoare și omul
// a cerut singur repetarea: nu ne băgăm.
function holdAtStart(video) {
  if (!video || video.loop) return;
  video.currentTime = 0;
  // Pauza doar dacă redarea a reluat DE LA ÎNCEPUT (reflexul playerului după derularea
  // noastră). Dacă între timp omul a sărit el în altă parte, a plecat intenționat.
  const once = () => {
  ...
  setTimeout(() => {
    video.removeEventListener('playing', once);
    if (!video.paused && video.currentTime < 3) video.pause(); // plasă, dacă playerul n-a emis „playing”
  }, 1500);
}
...
function onMessage(msg) {
  ...
  if (msg.type === 'CAPTURE_STATE') {
    if (msg.capturing) state.invoked = true; // captura a pornit => gestul a fost făcut
  ...
  if (msg.type === 'CAPTURE_FAILED') {
    ...
    // Plasă de siguranță: evidența din background a spus „invocat”, dar Chrome a refuzat.
...
function startPlayback() {
  ...
  state.lastSection = -1;
  stopPlayback(true);
  // Tot ce se afișează în Pasul 2 (acordul mare, banda, structura, foaia) pleacă de la
  // cronologia CURĂȚATĂ de intruși — altfel acordul mare spunea „Am” în timp ce foaia spunea
  // „A”. Memoria salvată rămâne brută: curățarea e reversibilă, o detecție ștearsă nu e.
  state.chords = cleanTimeline(state.chords, videoDuration());
  computeStructure();
  const tick = () => {
    const video = getVideo();
    if (video) {

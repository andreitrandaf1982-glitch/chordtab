...
import { matchChord, chordRelation } from '../lib/chords.js';
...
export const SMOOTHING = {
  ...
  // HISTEREZIS PE RUDE (audit 3, melodii reale): intrușii erau aproape toți rude ale acordului
  // comis — răsturnări de calitate pe aceeași fundamentală (Am în loc de A, F# în loc de F#m)
  // și relative (Bm lângă D, Em lângă G), scurte (0,8–1 s). O rudă trebuie să câștige mai mult
  // timp ȘI cu un avantaj de scor față de acordul comis ca s-o credem. Acordurile străine
  // (schimbările adevărate) rămân la regulile de mai sus — nu întârziem nimic real.
  // Valorile vin din grila `node tools/eval-real.mjs --grid` pe cele cinci melodii reale
  // (21.09.2026): media intrușilor 14,8 % → 11,4 %, ZERO tranziții reale ratate, 97,6 % din
  // timpul „bun” păstrat. Un hold general peste 0,45 s scădea intrușii mai mult, dar rata
  // schimbări reale (A → F#m e și relativă, și schimbare adevărată) — de aceea NU.
  qualityHoldFactor: 2,    // A → Am: minHold × 2 = 0,9 s
  relativeHoldFactor: 1.5, // G → Em: minHold × 1,5 ≈ 0,7 s
  kinMargin: 0.05,         // și scorul rudei trebuie să-l bată pe al acordului comis cu atât
};
...
export class Analyzer {
  ...
  evaluate(now) {
    ...
    const { label, score, scores } = matchChord(avg, { bass: avgBass, scores: true });
    ...
    const kin = chordRelation(this.committed, label);
    const hold = this.cfg.minHoldSeconds
      * (kin === 'quality' ? this.cfg.qualityHoldFactor : kin === 'relative' ? this.cfg.relativeHoldFactor : 1);
    if (now - this.candidate.sinceT < hold) return;
    // O rudă câștigă doar dacă șablonul ei e clar mai bun decât al acordului comis ACUM.
    if (kin && score - (scores[this.committed] ?? 0) < this.cfg.kinMargin) return;
